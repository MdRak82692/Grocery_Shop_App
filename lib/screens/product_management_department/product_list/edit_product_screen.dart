...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  void initState() {
    ...
    _pricePerProductController =
        TextEditingController(text: widget.product.pricePerProduct.toString());
    _fetchCategories();
  }
  ...
  Future<void> _updateProduct() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final existingProduct = await collection
            .findOne(mongo.where.eq('productName', productName));

        if (existingProduct != null &&
            existingProduct['_id'] != widget.product.id) {
          _showErrorDialog(
              'Product Name already exists. Please Enter New Product Name.');
        } else {
          await collection.update(
            mongo.where.id(widget.product.id),
            mongo.modify
                .set('productName', productName)
                .set('categoryName', _selectedCategory)
                .set('pricePerProduct', pricePerProduct),
          );
        ...
        _showSucessDialog('Product List Data has been Updated Successfully.');
      } catch (e) {
        _showErrorDialog('Failed to update product. Error: $e');
      }
    }
  }
                ...
                children: [
                  ...
                  ElevatedButton(
                    ...
                    child: Container(
                      ...
                      child: Text(
                        'Update Product List Information',
                        style: TextStyle(
                          fontSize: 20,
                          fontFamily: 'RobotoCondensed',
                          fontWeight: FontWeight.bold,
